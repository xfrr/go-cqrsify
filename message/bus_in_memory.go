...
type handlerFn func(ctx context.Context, msg Message) error

// InMemoryBus is an in-memory implementation of the message bus.
type InMemoryBus struct {
	mu              sync.RWMutex
	handlers        map[string]Handler[Message] // Store original handlers
...
func (b *InMemoryBus) Dispatch(ctx context.Context, msg Message) error {
	t := reflect.TypeOf(msg).Name()
	b.mu.RLock()
	handler, ok := b.wrappedHandlers[t]
	b.mu.RUnlock()
	if !ok {
		return fmt.Errorf("no handler registered for message type %s", t)
	}

	return handler(ctx, msg)
}

func (b *InMemoryBus) RegisterHandler(msgType string, handler Handler[Message]) error {
	b.mu.Lock()
	defer b.mu.Unlock()
	...
	b.wrappedHandlers[msgType] = func(ctx context.Context, msg Message) error {
...
func (b *InMemoryBus) applyMiddlewares(handler Handler[Message]) Handler[Message] {
